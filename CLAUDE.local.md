## Extends CLAUDE.md — Communication Style
...
## Extends CLAUDE.md — Task Files (HPW-only)

Plan/task files: name `YYYY-MM-DD-NN[-slug].md`, default location `tmp/tasks/todo/`. Full format (5-row header + sections) in `rules/cts/task-authoring.md`. Move to `tmp/tasks/done/` when completed.
