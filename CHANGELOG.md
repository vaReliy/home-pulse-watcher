...
- **Masked raw Telegram ID in `start.handler.ts` registration log.** `apps/api/src/modules/telegram/handlers/start.handler.ts` logged the raw Telegram numeric user ID via `logger.log(\`New user registered: ${telegramId}\`)`— a stable per-user identifier, the same class of gap already fixed in`maskMac()`below but for this sibling call site (follow-up task emitted by the MAC-masking`security-scanner`audit). Added`maskTelegramId()`(last-4-chars`...XXXX`, `\*\*\*`fallback for short input, same shape as`maskMac()`); new test asserts the raw ID never appears in log output across both branches. `tester`+`reviewer`+`security-scanner`gate: 0 Fix Now. This is now the third independent copy of the same masking pattern (after`maskMac()`and, pending, a`notification-dispatcher.ts`chatId site the security-scanner audit also found) — two follow-up tasks emitted: mask the`notification-dispatcher.ts`chatId log, and extract a shared`maskTrailing`/`maskPii` util now that a third site exists.
